...
Result<RegResult> run(const PointCloud&, const PointCloud&, const RegOptions&) {
    return makeError(ErrorCode::Unsupported,
                     "bufferx: needs the NPZ codec (vcpkg npz feature) for the handoff");
}
...
fs::path findScript() {
    std::error_code ec;
    if (const char* p = std::getenv("CLOUDCROPPER_BUFFERX_SCRIPT")) {
        if (fs::exists(p, ec)) return p;
    }
    const fs::path rel = "backend/registration/bufferx/python/bufferx_worker.py";
...
Result<void> writeNpz(const PointCloud& pc, const fs::path& path) {
    ...
    if (!sink.ok()) return makeError(ErrorCode::IoError, "bufferx: cannot create " + path.string());
    io::NpzWriter w;
...
Result<RegResult> run(const PointCloud& source, const PointCloud& target,
    ...
    if (script.empty())
        return makeError(ErrorCode::NotFound, "bufferx: bufferx_worker.py not found "
                                              "(set CLOUDCROPPER_BUFFERX_SCRIPT)");

    const auto cfg = configValues("bufferx.yaml");
    ...
    if (const std::string wd = get("weights_dir", ""); !wd.empty())
        ::setenv("CLOUDCROPPER_BUFFERX_WEIGHTS", wd.c_str(), /*overwrite=*/1);
    ...
    static TempDirGuard tmpGuard{
        ...
        ("cc_bufferx_" + std::to_string(static_cast<long>(::getpid())))};
    ...
    const std::string voxelSize  = opt.bufferxVoxel > 0 ? std::to_string(opt.bufferxVoxel)
                                                        : get("voxel_size", "0.0");
    const auto&       pos        = target.positions();
    // Target-feature cache key: geometry hash + downsample voxel + device.
    ...
    // to the worker: the optional GICP refine is chained in C++ below (§3.4).
    ...
    if (!t || t->array.size() != 16)
        return makeError(ErrorCode::ParseError, "bufferx: worker returned no 4x4 transform");

    RegResult out;
    for (std::size_t i = 0; i < 16; ++i) out.transform[i] = t->array[i].asDouble();
    out.converged = res->find("converged") && res->find("converged")->asBool();
    // BUFFER-X does not produce the GPIS confidence / norm-residual signals, so
    // those RegResult fields keep their -1 (not-available) defaults.

    const std::string dev = res->find("device") ? res->find("device")->asString("?") : "?";
    std::ostringstream detail;
    detail << "BUFFER-X (worker, " << dev << "): ";
    ...
    // Surface the worker's fallback note (e.g. core/weights missing) so an
    ...
    // BufferXGicp: refine the coarse global pose with GICP (small_gicp, already
    // linked in C++) seeded by the BUFFER-X 4x4 — keeps the worker inference-only.
