...
class UnsafeKdTreeGeneric {
 public:
  ...
  using Index = Adaptor<kiss_matcher::L2_Simple_Adaptor<double, ThisClass, double>, ThisClass, 3, size_t>;

  /// @brief Constructor
  /// @param points  Input points
  explicit UnsafeKdTreeGeneric(const PointCloud &points)
    : points(points), index(3, *this, kiss_matcher::KDTreeSingleIndexAdaptorParams(10)) { index.buildIndex(); }
  ...
  size_t knn_search(const Eigen::Vector4d &pt,
                    size_t k,
                    size_t *k_indices,
                    double *k_sq_dists) const { return index.knnSearch(pt.data(), k, k_indices, k_sq_dists); }
  ...
  size_t radius_search(const Eigen::Vector4d &pt,
    ...
    params.sorted = true;  // In case of BUFFER, sorting is needed
...
class KdTreeGeneric {
 public:
  ...
  size_t knn_search(const Eigen::Vector4d &pt,
                    size_t k,
                    size_t *k_indices,
                    double *k_sq_dists) const { return tree.knn_search(pt, k, k_indices, k_sq_dists); }
